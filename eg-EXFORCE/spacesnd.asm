...
;
; This variant adds standard PC Speaker beeps on fire and hit.
; Build:
;   nasm -f bin spacesnd.asm -o spacesnd.com
...
ORIGINAL_PIT_DIVISOR equ 0087h
...
.not_escape:
    ...
    mov [di+0Ah], ax
    mov ax, 0470h ; or ORIGINAL_PIT_DIVISOR
    mov cx, 1
    call speaker_beep
...
.hit_dy_abs:
    ...
    mov ax, 0250h ; or ORIGINAL_PIT_DIVISOR
    mov cx, 2
    call speaker_beep
.next_hit:
    inc word [bp-24h]
...
speaker_beep:
    ; Standard PC Speaker tone:
    ;   PIT channel 2 frequency divisor in AX
    ;   rough duration in CX
    ; Preserves all general registers for the compact game loop.
    ;
    ; Audible conversion of the original PIT setup.
    ;
    ; Original space.com writes control 34h and divisor 0087h to PIT channel 0:
    ;   out 43h, 34h
    ;   out 40h, 87h
    ;   out 40h, 00h
    ;
    ; PC Speaker uses PIT channel 2, so this keeps the same mode/divisor idea
    ; but changes the channel and data port:
    ;   34h -> B4h, 40h -> 42h, then gate speaker through port 61h.
    ;
    ; AX = divisor, normally ORIGINAL_PIT_DIVISOR.
    ; CX = rough duration. PUSHA/POPA keeps game-loop registers intact.
    pusha
    mov bx, ax
    mov al, 0B6h ; 0B6h for square wave, 0B4h for same as original
    out 43h, al
    mov ax, bx
    out 42h, al
    mov al, ah
